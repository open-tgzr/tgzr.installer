...
def execute(cmd: list[str], echo, raises: bool = False, exits: bool = True) -> bool:
    try:
        ...
        echo(f"TypeError in cmd arg: {cmd}")
        raise
...
def install_requirements(
    ...
    if allow_prerelease:
        ...
        # If too annoying, we'll add a separate option like '--use-cache-with-prerelease'
...
def create_temp_venv(
    ...
    if python_version is not None:
        more_options.extend(["-p", python_version])
...
def create_home_folder(
    ...
    if additional_packages:
        more_options.extend(
            sum([["--additional-packages", name] for name in additional_packages], []),
        )
...
def gui_install(
    ...
    if not ok:
        echo(f"Could not install {requirements} !")
        return
...
def headless_install(
    ...
    if not ok:
        raise Exception("Could not create home folder :/")
